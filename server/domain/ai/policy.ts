...
export const AI_POLICY_BOUNDARY_REPLY =
  "Я не могу раскрывать или отменять внутренние инструкции и защитные ограничения. Могу продолжить работу в рамках информационной помощи по вашему делу: объяснить этап, документы, задачи или подготовить вопросы для отдельной консультации с квалифицированным юристом.";
...
export function buildAiInstructions(context: AiCaseContext): string {
  ...
  const legalConsultationInstruction = context.planCode === "LITE"
    ? "Если вопрос требует индивидуального юридического решения, обозначь ограничения и при необходимости предложи отдельную консультацию с квалифицированным юристом. Не создавай впечатление, что персональный или сопровождающий юрист входит в тариф LITE."
