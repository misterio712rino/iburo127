...
const DIRECT_RESTRICTED_ACTION_PATTERNS = [
  /\bподпиш(?:и|ите)\b/iu,
  /\bподписать\s+(?:за\s+меня|от\s+моего\s+имени)\b/iu,
  /\bотправ(?:ь|ьте)\b[^.!?\n]{0,120}\b(?:в\s+суд|суду|арбитраж)\b/iu,
  /\bподай\b[^.!?\n]{0,120}\b(?:в\s+суд|заявлен|ходатайств|жалоб)\b/iu,
  /\bзаключ(?:и|ите)\b[^.!?\n]{0,80}\bдоговор\b/iu,
  /\bпредстав(?:ь|ьте)\b[^.!?\n]{0,80}\b(?:меня|мои\s+интересы)\b[^.!?\n]{0,80}\bсуд/iu,
  /\bдай\b[^.!?\n]{0,80}\bокончательн(?:ое|ый)\b[^.!?\n]{0,80}\bюридическ/iu,
  /\bприми\b[^.!?\n]{0,80}\bокончательн(?:ое|ый)\b[^.!?\n]{0,80}\bрешен/iu,
] as const;
...
const PROHIBITED_COMPLETION_CLAIMS = [
  /\bя\s+(?:уже\s+)?(?:подписал|подписала|отправил|отправила|подал|подала|заключил|заключила)\b/iu,
  /\b(?:документ|документы|заявление|жалоба|ходатайство)\s+(?:уже\s+)?(?:отправлен[ыо]?|подан[ыо]?|подписан[ыо]?)\s+(?:мной|в\s+суд)/iu,
  /\bдоговор\s+(?:уже\s+)?заключ[её]н\s+(?:мной|от\s+вашего\s+имени)/iu,
] as const;
