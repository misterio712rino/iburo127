...
import { assertValidYandexStorageBucketName } from "@/server/files/yandex-storage-bucket-name";
...
export function readYandexObjectStorageConfig(
  ...
  let bucket: string;
  try {
    bucket = assertValidYandexStorageBucketName(requireEnv(env, "YANDEX_STORAGE_BUCKET"));
  } catch {
    throw new Error(`${PRODUCTION_CONFIG_ERROR}:YANDEX_STORAGE_BUCKET`);
  }
