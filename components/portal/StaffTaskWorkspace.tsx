...
export function StaffTaskWorkspace({
  ...
  if (!items.length) {
    return <StaffTaskCards items={items} />;
  }

  return (
    <div className="grid gap-4">
      <section
        className="rounded-[24px] border border-white/80 bg-white/85 p-4 shadow-[0_12px_40px_rgba(15,23,42,0.055)] sm:p-5"
        aria-label="Поиск и фильтры задач"
      >
          ...
          {FILTERS.map((item) => {
            ...
            return (
              ...
              >
                {item.label}
                <span
                  className={`tabular-nums ${active ? "text-white/75" : "text-slate-400"}`}
                  aria-hidden="true"
                >
                  {counts[item.value]}
                </span>
      ...
      <p className="text-xs font-semibold text-slate-400" role="status" aria-live="polite">
        Показано задач: {visibleItems.length}
      </p>
