...
type StaffSummary = {
  ...
  questionnaire: { completed: number; total: number; percent: number; status: keyof typeof STATUS_LABELS };
  practicum: { completed: number; total: number; percent: number; status: keyof typeof STATUS_LABELS };
  documents: { total: number; sentForReview: number; reviewed: number };
  readyFileCount: number;
  nextAction: { title: string; description: string; segment: string };
};

export function StaffProgressView({ caseHref, nextHref, caseNumber, summary }: { caseHref: string; nextHref: string; caseNumber: string; summary: StaffSummary }) {
  return (
