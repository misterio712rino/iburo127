  ...
  return (
    <div className="flex min-w-0 flex-col gap-7 py-1 sm:gap-9 sm:py-2">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <span className="inline-flex min-h-11 items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-xs font-semibold text-emerald-700">
          <ShieldCheck className="size-4" aria-hidden="true" />
          Защищённое хранилище
        </span>
      </header>

      {error ? <p role="alert" className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{error}</p> : null}

      <section className="relative overflow-hidden rounded-[32px] bg-slate-950 p-6 text-white shadow-[0_22px_60px_rgba(15,23,42,.14)] sm:p-8">
        <FileLock2 className="absolute -bottom-10 -right-8 size-56 opacity-[.06]" aria-hidden="true" />
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="text-xs font-bold uppercase tracking-[.16em] text-white/55">Добавить материал</p>
            <h2 className="mt-3 text-2xl font-semibold tracking-[-.035em] sm:text-3xl">Передайте файл в дело</h2>
            <p className="mt-3 text-sm leading-6 text-white/60">PDF, JPG, PNG, WEBP, DOC или DOCX до 50 МБ. После загрузки файл проходит проверку безопасности перед появлением в списке.</p>
          </div>
          <label className="inline-flex min-h-12 shrink-0 cursor-pointer items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5 hover:bg-slate-100 focus-within:ring-4 focus-within:ring-white/25 has-[input:disabled]:cursor-not-allowed has-[input:disabled]:opacity-60">
      ...
      <section aria-labelledby="ready-files-heading">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.14em] text-[#b9202b]">Хранилище</p>
        ...
        {files.length ? (
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {files.map((file) => (
              <article key={file.id} className="group rounded-[28px] border border-white/80 bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,.055)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_48px_rgba(15,23,42,.08)] sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#f4f1ef] text-[#b9202b]"><FileCheck2 className="size-5" aria-hidden="true" /></span>
                  <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-[11px] font-semibold text-emerald-700">Проверен</span>
                </div>
                ...
                <button
                  ...
                  className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#b9202b]/15 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                >
          ...
          <div className="mt-5 rounded-[28px] border border-dashed border-slate-200 bg-white/70 p-8 text-center shadow-[0_10px_30px_rgba(15,23,42,.03)]">
            <span className="mx-auto grid size-12 place-items-center rounded-2xl bg-slate-100 text-slate-400"><FileLock2 className="size-5" aria-hidden="true" /></span>
