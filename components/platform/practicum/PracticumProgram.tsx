...
export function PracticumProgram({ getStatus }: { getStatus: (id: string) => LessonStatus }) {
  return <div className="flex flex-col gap-5">{PRACTICUM_MODULES.map((module) => {
    ...
    return <section key={module.id} className="practicum-module-surface overflow-hidden rounded-[1.5rem] border border-border bg-card text-card-foreground shadow-[0_14px_40px_rgba(0,0,0,.045)]">
