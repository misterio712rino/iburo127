...
export function AiMessage({ message, onEscalate }: { message: AiMessageType; onEscalate: () => void }) {
  ...
  return <article className={cn("flex min-w-0 gap-3", !assistant && "justify-end")}>
    ...
    <div className={cn("min-w-0 max-w-[88%] rounded-[1.3rem] px-4 py-3 sm:max-w-[78%] sm:px-5 sm:py-4", assistant ? "border border-border bg-muted/55" : "bg-primary text-primary-foreground")}>
      ...
      {message.action?.href ? <Button render={<Link href={message.action.href} />} nativeButton={false} variant="outline" className="mt-4 h-9 max-w-full rounded-full bg-background px-4 text-foreground">{message.action.label}</Button> : null}
      {message.action?.type === "ASK_LAWYER" ? <Button type="button" variant="outline" className="mt-4 h-9 max-w-full rounded-full bg-background px-4 text-foreground" onClick={onEscalate}>{message.action.label}</Button> : null}
    </div>
