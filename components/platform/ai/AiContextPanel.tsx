...
export function AiContextPanel({ context, escalated, onEscalate }: { context: AiContext; escalated: boolean; onEscalate: () => void }) {
  ...
  return <div className="grid gap-4">
    ...
    <PlatformCard className="p-5 sm:p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-primary">Демо-действие</p><h2 className="mt-3 text-lg font-semibold">Нужна оценка специалиста?</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Подготовьте вопрос для {context.assignedLawyer}. Отправка в реальную систему не выполняется.</p><Button type="button" variant="outline" className="mt-5 h-10 w-full rounded-full whitespace-normal" onClick={onEscalate}>{escalated ? "Вопрос подготовлен" : `Задать вопрос ${context.assignedLawyer}`}</Button>{escalated ? <p role="status" className="mt-3 text-xs leading-5 text-primary">В демонстрационном режиме вопрос подготовлен для передачи специалисту.</p> : null}</PlatformCard>
  </div>;
}
