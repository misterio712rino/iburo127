...
function AiChat({ identity }: { identity: DemoIdentity }) {
  ...
  const { conversation, appendUser, appendReply, escalate, reset } = useAiConversation(identity.id, context);
  const [typing, setTyping] = useState(false);
  const timer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const endRef = useRef<HTMLDivElement>(null);

  useEffect(() => () => { if (timer.current) clearTimeout(timer.current); }, []);
  useEffect(() => { endRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" }); }, [conversation.messages.length, typing]);
  ...
  return <div className="flex min-w-0 flex-col gap-6 sm:gap-8">
    <SectionHeader title="AI-помощник" description="Поможет разобраться в текущем этапе и работе платформы." action={<Button type="button" variant="ghost" className="w-fit rounded-full" onClick={clearChat}><RotateCcw data-icon="inline-start" />Очистить демо-чат</Button>} />
    <div className="flex gap-3 rounded-[1.2rem] border border-primary/25 bg-primary/10 p-4 text-sm leading-6"><ShieldCheck className="mt-0.5 size-5 shrink-0 text-primary" aria-hidden="true" /><div><p className="font-semibold">Демонстрационный AI-режим</p><p className="mt-1 text-muted-foreground">Ответы показывают предполагаемую логику работы AI-помощника и не являются юридическим заключением.</p></div></div>
    <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_20rem]">
      <PlatformCard className="flex min-w-0 flex-col overflow-hidden">
        ...
        <div role="log" aria-live="polite" aria-label="История диалога" className="max-h-[36rem] min-h-[24rem] min-w-0 space-y-5 overflow-y-auto p-3 sm:p-5">
          {conversation.messages.map((message) => <AiMessage key={message.id} message={message} onEscalate={escalate} />)}
      ...
      <AiContextPanel context={context} escalated={Boolean(conversation.escalatedAt)} onEscalate={escalate} />
    </div>
  </div>;
}
