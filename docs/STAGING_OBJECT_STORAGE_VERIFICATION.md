...
The command refuses to run unless `IB_STORAGE_TARGET` is exactly `staging` and the configured application bucket exactly matches `IB_STAGING_STORAGE_BUCKET`.

## Read-only checks

The verifier performs:

1. `HeadBucket` — bucket exists and the service account can reach it.
2. `GetBucketAcl` — rejects grants to the S3 public groups `AllUsers` and `AuthenticatedUsers`.
3. `GetBucketPolicy` — absence of a policy is accepted. A deny-only policy is accepted. **Any `Allow` statement fails closed as `STAGING_STORAGE_POLICY_REVIEW_REQUIRED`** because principal, resource and condition semantics require explicit human review before the bucket can be called private. Policy JSON and principal identities are never printed.
4. `GetBucketCors` — requires the exact staging origin, rejects wildcard or additional origins, and requires browser `PUT` with `Content-Type` for the existing direct signed-upload flow.
...
Production bucket changes remain prohibited without explicit production approval.
