...
async function verifyMobileDrawer(page, label, viewport) {
  ...
  if (!controlsId || /[\u0000-\u001f\u007f]/u.test(controlsId)) {
    throw new Error(`${label}: menu trigger has no usable aria-controls target`);
  }

  await trigger.click();
  const controlledSelector = await page.evaluate((id) => `#${CSS.escape(id)}`, controlsId);
  const controlled = page.locator(controlledSelector);
  await controlled.waitFor({ state: "visible", timeout: 10_000 });

  let dialog = controlled;
  if (await controlled.getAttribute("role") !== "dialog") {
    dialog = controlled.locator("xpath=ancestor::*[@role='dialog'][1]");
  }
  await dialog.waitFor({ state: "visible", timeout: 10_000 });
  if (await dialog.getAttribute("role") !== "dialog") throw new Error(`${label}: controlled drawer has no dialog container`);
  ...
  checks.push({ kind: "mobile-drawer-focus", label, accessibleName, controlsId, pass: true });
}
