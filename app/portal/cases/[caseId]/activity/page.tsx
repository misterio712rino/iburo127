...
const TYPE_LABELS: Record<string, string> = {
  ...
  "file.upload.registered": "Загрузка файла зарегистрирована",
  "file.download.authorized": "Скачивание файла разрешено",
                ...
                {event.metadata && Object.keys(event.metadata).length ? (
                  <dl className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
                    {Object.entries(event.metadata).map(([key, value]) => (
                      <div key={key} className="rounded-xl bg-slate-50 px-3 py-2 text-xs">
                        <dt className="text-slate-400">{key}</dt>
                        <dd className="mt-0.5 font-semibold text-slate-700">{String(value)}</dd>
                      </div>
                    ))}
                  </dl>
                ) : null}
