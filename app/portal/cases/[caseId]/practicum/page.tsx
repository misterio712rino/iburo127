...
export default async function PortalPracticumPage({ params }: { params: Promise<{ caseId: string }> }) {
  ...
  return (
    <CasePortalFrame sessionProvider={sessionProvider} actor={actor} clientCase={clientCase} sectionLabel="Практикум" showStaffTasks={isStaff}>
        ...
        {isStaff ? (
          ...
          <>
            <ClientCaseModuleIntro
              ...
              title="Практикум"
              description="Проходите материалы в удобном темпе. Выполненные уроки сохраняются в вашем деле, а при открытии с другого устройства отображается актуальный прогресс."
              icon={GraduationCap}
            />
            <div className="mt-6">{practicumView}</div>
          </>
        )}
      </div>
    </CasePortalFrame>
